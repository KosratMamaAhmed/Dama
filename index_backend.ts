...
router.post('/api/room/create', async (request, env) => {
  try {
    ...
    await env.gama_kv.put(`room:${roomCode}`, JSON.stringify(roomState), { expirationTtl: 7200 });
...
router.post('/api/room/join', async (request, env) => {
  try {
    ...
    const roomRaw = await env.gama_kv.get(`room:${roomCode}`);
    if (!roomRaw) {
    ...
    await env.gama_kv.put(`room:${roomCode}`, JSON.stringify(room), { expirationTtl: 7200 });
...
router.post('/api/invite/send', async (request, env) => {
  try {
    ...
    await env.gama_kv.put(`invite:${targetUsername.toLowerCase()}`, JSON.stringify(inviteData), { expirationTtl: 300 });
...
router.get('/api/invite/poll', async (request, env) => {
  try {
    ...
    const inviteRaw = await env.gama_kv.get(`invite:${username.toLowerCase()}`);
    if (!inviteRaw) {
...
router.post('/api/invite/clear', async (request, env) => {
  try {
    ...
    if (username) {
      await env.gama_kv.delete(`invite:${username.toLowerCase()}`);
    }
    return corsResponse({ success: true });
  } catch (err: any) {
    return corsResponse({ error: err.message }, 500);
